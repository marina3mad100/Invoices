...
@section('content')
<div class="p-6">
    <h1 class="text-2xl font-semibold mb-6 dark:text-gray-200">Edit a Invoice</h1>
	...
	<form id="user-item-form" class="space-y-6"  method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input
                    type="text"  value="{{ $invoice->number }}"
            ...
            <div>
                ...
                <input
                    type="text"   value="{{ $invoice->amount }}"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        users=users.map(function(item) {
                return {'value':item.user.id , 'label':item.first_name+' '+item.last_name , 'selected':item.user.id == user_id ? true : false};
        })	;
        console.log(user_id);
        console.log(users);
     ...
     $("#user-item-form").on("submit", function(event) {
        let url = "{{ route('invoices.update', [':id']) }}".replace(':id', $('#id').val());
